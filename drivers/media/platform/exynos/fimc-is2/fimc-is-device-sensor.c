...
static int get_sensor_mode(struct fimc_is_device_sensor *device,
...
int fimc_is_sensor_gpio_on(struct fimc_is_device_sensor *device)
{
	...
	if (!core) {
		merr("%s core is NULL", device, __func__);
		ret = -ENODEV;
		goto p_err;
	}
...
static void fimc_is_sensor_dtp(unsigned long data)
{
	...
	if ((framemgr->num_frames == 0) || (framemgr->num_frames > FRAMEMGR_MAX_REQUEST)) {
		err("frame count of framemgr is invalid(%d)", framemgr->num_frames);
		return;
	}
...
static int fimc_is_sensor_notify_by_fstr(struct fimc_is_device_sensor *device, void *arg)
{
	...
	if (frame) {
		...
		if (frame->has_fcount) {
			...
			list_for_each(temp, &framemgr->queued_list[FS_PROCESS]) {
	...
	void *arg)
{
	...
	case CSI_NOTIFY_VSYNC:
		...
		break;
	case CSI_NOTIFY_FLASH:
		ret = v4l2_subdev_call(device->subdev_module, core, ioctl, V4L2_CID_SENSOR_NOTIFY_FLASH_FIRE, arg);
		if (ret)
			merr("fimc is sensor notify flash fire is fail", device);
		break;
	case CSI_NOTIFY_ACTUATOR:
		ret = v4l2_subdev_call(device->subdev_module, core, ioctl, V4L2_CID_SENSOR_NOTIFY_ACTUATOR, arg);
		if (ret)
			merr("fimc is sensor notify flash fire is fail", device);
		break;
	case CSI_NOTIFY_M2M_ACTUATOR:
		ret = v4l2_subdev_call(device->subdev_module, core, ioctl, V4L2_CID_SENSOR_NOTIFY_M2M_ACTUATOR, arg);
		if (ret)
			merr("fimc is sensor notify flash fire is fail", device);
		break;
	case SENSOR_NOTIFY_ACTUATOR_INIT:
		ret = v4l2_subdev_call(device->subdev_module, core, ioctl, V4L2_CID_SENSOR_NOTIFY_ACTUATOR_INIT, arg);
		if (ret)
			merr("fimc is sensor notify actuator init fail", device);
		break;
#endif
	}
}
